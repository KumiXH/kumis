...
本文件把候选机会转换为可讨论的技术卡。卡片中的训练损失权重均为工程起始假设，除非来源明确披露，否则不能作为论文原始配置。

## DD-01 动态星芒与镜头眩光视频化 (Temporal starburst and lens-flare rendering)
- 能力族：computational_optics；模式：online_recording；真实性边界：perceptual
- 证据：official_apple_cinematic_mode, local_fluxir
...
- 数据制作：真实夜景/舞台视频；合成多种光圈、鬼影和星芒参数；加入不同曝光、镜头污渍和 rolling-shutter 扰动。
- LOSS/目标：L = L_render_recon + 0.2 L_temporal_warp + 0.1 L_highlight_shape + 0.05 L_flicker；权重为起始假设，必须做梯度与消融。
...
- 能力族：relighting；模式：offline_device；真实性边界：generative
- 证据：local_fluxir, local_authface, official_capcut_video_cutout
...
- 数据制作：HDR 人像、多光源人像、真实手机视频；用光照可控 3D 人脸和渲染数据补充主光方向、颜色、强度标签。
- LOSS/目标：重建 L1/Charbonnier + ROI perceptual + identity cosine + temporal warp + boundary alpha + illumination smoothness；身份损失只作用于脸部高层 embedding。
...
- 能力族：depth_focus；模式：offline_device；真实性边界：perceptual
- 证据：official_google_cinematic_blur, local_fluxsr, local_tiger
...
- 数据制作：真实双摄/多摄视频、带深度相机数据、电影镜头数据；合成不同 aperture、focus distance 和运动模糊。
- LOSS/目标：深度 L1 + alpha boundary + perceptual + temporal consistency + blur-kernel regularization；对发丝设置高权重边界采样。
...
- 能力族：temporal_exposure；模式：offline_device；真实性边界：generative
- 证据：local_flashvsr, local_wfvae
...
- 数据制作：真实短曝光序列合成长曝光目标；真实车流、瀑布、舞蹈和光轨视频；加入遮挡与曝光变化。
- LOSS/目标：区域重建 + motion-streak prior + temporal warp + subject identity/edge loss + background smoothness。
...
- 能力族：virtual_camera；模式：offline_device；真实性边界：generative
- 证据：local_fluxsr, local_fluxir, local_dit
...
- 数据制作：多摄/多视角视频、室内外静态场景和缓慢运动视频；合成相机轨迹、孔洞和视差变化。
- LOSS/目标：多视角 photometric + depth consistency + perceptual + hole-boundary + trajectory smoothness。
...
- 能力族：multi_camera；模式：online_recording；真实性边界：faithful
- 证据：official_apple_cinematic_mode, official_blackmagic_gen5_color, official_samsung_super_steady
...
- 数据制作：同步多摄视频，覆盖不同光照和焦段；采集镜头 response、噪声和畸变标定数据。
- LOSS/目标：跨摄颜色差异 + tone continuity + temporal flow + edge sharpness consistency + switch transient penalty。
...
- 能力族：motion_quality；模式：online_recording；真实性边界：faithful
- 证据：official_gopro_hypersmooth, official_dji_rocksteady, local_flashvsr
...
- 数据制作：真实运动视频与 IMU、合成抖动轨迹、人体/车辆运动数据；加入 rolling shutter 和遮挡。
- LOSS/目标：camera reprojection + non-rigid flow smoothness + subject shape preservation + horizon/line stability + temporal consistency。
...
- 能力族：night_hdr；模式：online_recording；真实性边界：faithful
- 证据：official_google_video_boost, local_flashvsr, local_wfvae
...
- 数据制作：真实夜景视频、短曝光到长曝光配对、合成 sensor noise/ISO/色偏/高光裁切；保留真实 motion。
- LOSS/目标：Charbonnier + perceptual + temporal warp + chroma noise + highlight roll-off + exposure trajectory loss。
...
- 能力族：portrait_identity；模式：online_recording；真实性边界：perceptual
- 证据：local_tiger, local_svfr, local_authface
...
- 数据制作：真实人像视频、VFH​​Q/CelebV-HQ/FFHQ 类静态数据、合成压缩/模糊/噪声/遮挡；加入不同年龄、肤色、妆容。
- LOSS/目标：区域重建 + ROI perceptual + ArcFace/identity cosine + temporal feature consistency + mask boundary + skin-color stability。
...
- 能力族：scene_editing；模式：offline_device；真实性边界：generative
- 证据：official_adobe_content_aware_fill_video, official_davinci_object_removal, official_davinci_magic_mask
...
- 数据制作：DAVIS/YouTube-VOS 类视频 mask、真实路人/电线采集、合成对象插入和镜头运动；对背景动态单独采样。
- LOSS/目标：known-region reconstruction + hole perceptual + temporal warp + boundary gradient + object-removal consistency。
...
- 能力族：color_science；模式：online_recording；真实性边界：perceptual
- 证据：official_panasonic_real_time_lut, official_blackmagic_gen5_color, official_sony_s_cinetone
...
- 数据制作：多相机、多光照、参考电影片段；采集颜色 chart、肤色和不同 ISO 的颗粒统计。
- LOSS/目标：色彩重建 + skin-tone deviation + look perceptual + temporal color stability + grain spectrum matching。
...
- 能力族：audio_visual；模式：online_recording；真实性边界：faithful
- 证据：official_blackmagic_camera_app, official_sony_real_time_tracking, official_insta360_me_mode
...
- 数据制作：多说话人采访、播客、会议和嘈杂环境；标注说话人-脸对应、镜头选择和用户偏好。
- LOSS/目标：speaker-face matching + focus/framing classification + trajectory smoothness + shot-switch penalty。
...
- 能力族：generative_video；模式：offline_device；真实性边界：generative
- 证据：local_fluxir, local_fluxsr, local_dit, local_magvit
...
- 数据制作：多视角/宽幅视频裁切任务、合成画外区域、动态对象和遮挡；使用区域 mask 作为 supervision。
- LOSS/目标：known-region identity + generated-region perceptual + temporal consistency + layout/depth consistency + boundary blend。
...
- 能力族：capture_assistance；模式：offline_device；真实性边界：faithful
- 证据：official_google_video_boost, local_flashvsr, local_wfvae
...
- 数据制作：从同一段真实视频产生不同 proxy 和高质量目标；覆盖中断、掉帧、温度降级和存储受限。
- LOSS/目标：proxy-to-GT restoration + metadata consistency + temporal quality + rate/storage regularization。
...
- 能力族：computational_optics；模式：online_recording；真实性边界：faithful
- 证据：official_davinci_magic_mask, official_adobe_roto_brush, local_fluxir
...
- 数据制作：真实雨滴/玻璃拍摄、可控喷雾和污渍数据、合成反射/透射层；保留透明和半透明边界。
- LOSS/目标：mask-aware reconstruction + temporal warp + transparency consistency + background edge loss。
...
- 数据制作：多人 HDR 人像、真实补光视频、3D human relighting 数据；合成不同人数和遮挡关系。
- LOSS/目标：person ROI reconstruction + identity + inter-person color consistency + occlusion boundary + temporal track consistency。
...
- 能力族：depth_focus；模式：offline_device；真实性边界：perceptual
- 证据：official_google_cinematic_blur, local_fluxsr
...
- 数据制作：城市/建筑/风景视频、真实 tilt-shift 镜头参考、深度标注和合成焦平面。
- LOSS/目标：depth-layer consistency + edge/alpha + perceptual style + temporal stability。
...
- 能力族：temporal_exposure；模式：online_recording；真实性边界：faithful
- 证据：official_dji_subject_tracking, local_flashvsr
...
- 数据制作：体育、儿童、宠物、舞台动作；标注事件边界和用户认为值得慢放的片段。
- LOSS/目标：interpolation reconstruction + flow consistency + occlusion handling + event boundary smoothness。
...
- 能力族：virtual_camera；模式：online_recording；真实性边界：faithful
- 证据：official_apple_cinematic_mode, official_sony_real_time_tracking
...
- 数据制作：双人/多人对话，标注焦点选择、切换时机和用户偏好；加入遮挡、转头和抢话。
- LOSS/目标：speaker association + focus target classification + transition smoothness + wrong-switch penalty。
...
- 能力族：multi_camera；模式：offline_device；真实性边界：faithful
- 证据：local_flashvsr, official_samsung_super_steady
...
- 数据制作：同步双摄运动视频、真实模糊/噪声、不同焦段标定；训练时随机失配和曝光差。
- LOSS/目标：cross-view photometric + reconstruction + confidence calibration + temporal consistency + edge detail。
...
- 能力族：motion_quality；模式：online_recording；真实性边界：faithful
- 证据：local_flashvsr, official_gopro_horizon_lock
...
- 数据制作：真实高速旋转视频、IMU 对齐数据、合成 row-time distortion；加入不同读出速度。
- LOSS/目标：geometric reprojection + line straightness + horizon stability + temporal warp + crop penalty。
...
- 能力族：night_hdr；模式：online_recording；真实性边界：perceptual
- 证据：local_authface, local_tiger, official_google_video_boost
...
- 数据制作：逆光人像视频、多曝光配对、不同肤色和光源；合成高光裁切、噪声和压缩。
- LOSS/目标：ROI reconstruction + identity + skin-tone + temporal color + highlight roll-off + boundary consistency。
...
- 能力族：portrait_identity；模式：offline_device；真实性边界：perceptual
- 证据：local_authface, local_svfr
...
- 数据制作：戴眼镜人像视频、可控反光拍摄、不同镜片和姿态；人工标注眼睛/镜片/高光。
- LOSS/目标：eye reconstruction + identity + reflection sparsity + temporal consistency + iris boundary。
...
- 能力族：scene_editing；模式：offline_device；真实性边界：generative
- 证据：official_capcut_video_cutout, local_fluxir
...
- 数据制作：多天气天空视频、真实地面光照和人物边界；合成天光方向、雾和动态云。
- LOSS/目标：sky reconstruction + boundary alpha + color/illumination consistency + temporal motion + semantic preservation。
...
- 能力族：color_science；模式：online_recording；真实性边界：perceptual
- 证据：official_blackmagic_gen5_color, official_panasonic_real_time_lut, official_sony_s_cinetone
...
- 数据制作：多摄同步拍摄 color chart、人像和不同光线；跨镜头 reference look 标定。
- LOSS/目标：skin-tone deviation + cross-lens color + temporal look smoothness + grain spectrum。
...
- 能力族：audio_visual；模式：online_recording；真实性边界：faithful
- 证据：official_dji_active_track, official_blackmagic_camera_app
...
- 数据制作：真实生活和运动声音、不同噪声场景、事件边界标注；用用户选择作为 ranking supervision。
- LOSS/目标：event classification + temporal boundary + highlight ranking + false-trigger penalty。
...
- 能力族：generative_video；模式：cloud_render；真实性边界：generative
- 证据：local_dit, local_magvit, local_fluxir
...
- 数据制作：视频局部编辑数据、mask/文本/参考图三元组、区域冻结训练；加入文字、人脸和动作保护。
- LOSS/目标：protected-region reconstruction + editable-region diffusion objective + temporal consistency + identity/text preservation。
...
- 能力族：capture_assistance；模式：online_recording；真实性边界：faithful
- 证据：local_flashvsr, local_wfvae, official_google_video_boost
...
- 数据制作：不同设备、温度、负载和片段长度的 profiling 数据；建立质量-延迟-功耗曲线。
- LOSS/目标：任务质量 + temporal stability + latency/power regularizer；训练权重不代替真实设备 profiling。
...
- 能力族：computational_optics；模式：online_recording；真实性边界：faithful
- 证据：official_davinci_magic_mask, local_flashvsr
...
- 数据制作：真实雨滴、指纹、灰尘、贴膜和逆光高光；hard negative 包含水面反光和灯光。
- LOSS/目标：mask BCE/Dice + temporal mask consistency + false-positive penalty + transparency regression。
...
- 能力族：relighting；模式：offline_device；真实性边界：generative
- 证据：official_dji_subject_tracking, local_fluxir
...
- 数据制作：舞台/演唱会视频、光源轨迹和人群遮挡；合成不同 spotlight 半径和颜色。
- LOSS/目标：track consistency + light falloff + boundary/occlusion + temporal intensity + perceptual style。
